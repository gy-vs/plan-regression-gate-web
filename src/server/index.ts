...
const rows: RecordRow[] = [
  {id:'alpha',name:'Primary plan comparisons',revision:3,content:'plan comparisons: alpha\nstate: active',updatedAt:new Date(0).toISOString()},
  {id:'beta',name:'Secondary plan comparisons',revision:5,content:'plan comparisons: beta\nstate: review',updatedAt:new Date(1000).toISOString()},
];

export function createApp(){
  const app=express();
  app.use(express.json({limit:'1mb'}));
  app.get('/api/bootstrap',(_req,res)=>res.json({family:"query-plan",count:rows.length}));
